...
public class FileAndDirectoryInfo {
    public static void main(String[] args) throws IOException
   {
      ...
      Path path = Paths.get(input.nextLine());  // create Path object based on user input
      ...
      if (Files.exists(path)) // if path exists, output info about it
      {
         ...
         if (Files.isDirectory(path)) // output directory listing
         {
            ...
            DirectoryStream<Path> directoryStream = Files.newDirectoryStream(path); // object for iterating through a directory's contents
